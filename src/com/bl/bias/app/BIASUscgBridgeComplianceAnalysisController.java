...
public class BIASUscgBridgeComplianceAnalysisController 
{
	...
	private void runTask() throws InterruptedException
	{
		...
		if (continueAnalysis)
		{
			...
			Integer marinePeriodsPerWeek = 0;
			Double marineAccessPeriodSpanAsSerial = 0.0;
			if ((bridgeComboBox.getSelectionModel().getSelectedIndex() == 0) && (BIASUscgBridgeComplianceAnalysisConfigPageController.getIncludeMarineHighUsagePeriodsBridge1()))
			{
				...
				marinePeriodsPerWeek = BIASUscgBridgeComplianceAnalysisConfigPageController.getMarinePeriodsPerWeekBridge1AsInteger();
				marineAccessPeriodSpanAsSerial = (BIASUscgBridgeComplianceAnalysisConfigPageController.getMarineAccessPeriodSpanBridge1AsDouble() / 24);
			...
			else if ((bridgeComboBox.getSelectionModel().getSelectedIndex() == 1) && (BIASUscgBridgeComplianceAnalysisConfigPageController.getIncludeMarineHighUsagePeriodsBridge2()))
			{
				...
				marinePeriodsPerWeek = BIASUscgBridgeComplianceAnalysisConfigPageController.getMarinePeriodsPerWeekBridge2AsInteger();
				marineAccessPeriodSpanAsSerial = (BIASUscgBridgeComplianceAnalysisConfigPageController.getMarineAccessPeriodSpanBridge2AsDouble() / 24);
			}
			...
			Boolean includeSummaryResultsOnSpreadsheet = false;
			...
			if (BIASGeneralConfigController.getUseSerialTimeAsFileName())
			{
				WriteBridgeComplianceFiles3 filesToWrite = new WriteBridgeComplianceFiles3(analyzeData.getClosures(), bridge.getValue() + readData.getDateSpan(), textArea.getText(), getSaveFileFolderForSerialFileName(), includeHighUsePeriods, includeViolationsOnClosureSheets, 
						includeConfidentialityDisclosure, includeSummaryResultsOnNotepad, includeSummaryResultsOnSpreadsheet, marineAccessPeriodStartHour, marineAccessPeriodEndHour, marinePeriodsPerWeek, marineAccessPeriodSpanAsSerial);
