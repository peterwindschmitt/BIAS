...
public class WriteTTestFiles1
{
	private static LocalTime startWriteFileTime;
	...
	private static XSSFSheet velocityTTestSheet; 
	
	private static IndexedColors[] tTestColors = new IndexedColors[8];
	
	public WriteTTestFiles1(String textAreaContents, HashSet<ResultFromTTest> byTypeTTestResults, HashSet<ResultFromTTest> byGroupTTestResults)
	{
		...
		startWriteFileTime = LocalTime.now();
		resultsMessage = "\nStarted writing output file at "+startWriteFileTime;
		workbook = new XSSFWorkbook();
		
		if (TTestValidateTTest.getGenerateVelocity())
		{
	...
	public void writeFiles1()
	{
		...
		{
			velocityTTestSheet = workbook.createSheet("Velocity");
			velocityTTestSheet.setDisplayGridlines(false);
			Row row;
			Cell cell; 
			
			// Write header here
			// First row of sheet header
			row = velocityTTestSheet.createRow(rowNumber);
			...
			if (byGroupTTestResults.size() > 0)
			{
				...
				if (BIASTtestConfigPageController.getShowSlightAndEqualTstatColumns())
				{
					velocityTTestSheet.addMergedRegion(new CellRangeAddress(rowNumber, rowNumber, 0, 10));
				}
				else
				{
					velocityTTestSheet.addMergedRegion(new CellRangeAddress(rowNumber, rowNumber, 0, 8));
				}

				cell = row.createCell(0);
				cell.setCellStyle(style9);
				cell.setCellValue("Velocity - Group Tests");
				...
				setRegionBorderThin(new CellRangeAddress(rowNumber, rowNumber, cell.getColumnIndex(), cell.getColumnIndex()), velocityTTestSheet);
				...
				cell.setCellValue("Velocity Mean Diff");
				setRegionBorderThin(new CellRangeAddress(rowNumber, rowNumber, cell.getColumnIndex(), cell.getColumnIndex()), velocityTTestSheet);
				...
				{
					if ((sortedGroupFiles.get(i).getVelocityA() != null)  && (sortedGroupFiles.get(i).getVelocityB() != null))
					{
						resultsSetSize++;
						rowNumber++;
						row = velocityTTestSheet.createRow(rowNumber);
						...
						if (sortedGroupFiles.get(i).getVelocityB() > sortedGroupFiles.get(i).getVelocityA())
						{
							cell.setCellValue(sortedGroupFiles.get(i).getAbsMeanVelocityDiff());
						}
						else
						{
							cell.setCellValue(-1 * sortedGroupFiles.get(i).getAbsMeanVelocityDiff());
						}
						setRegionBorderThin(new CellRangeAddress(rowNumber, rowNumber, cell.getColumnIndex(), cell.getColumnIndex()), velocityTTestSheet);

						cell = row.createCell(3);
						cell.setCellStyle(style5);
						if (Double.isInfinite(sortedGroupFiles.get(i).getVelocityTValue()))
						{
							cell.setCellValue("infinity");
						}
						else if (Double.isNaN(sortedGroupFiles.get(i).getVelocityTValue()))
						{
							cell.setCellValue(0.00);
						}
						else if (sortedGroupFiles.get(i).getVelocityB() > sortedGroupFiles.get(i).getVelocityA())
						{
							cell.setCellValue(sortedGroupFiles.get(i).getVelocityTValue());
						}
						else
						{
							cell.setCellValue(-1 * sortedGroupFiles.get(i).getVelocityTValue());
						}
						setRegionBorderThin(new CellRangeAddress(rowNumber, rowNumber, cell.getColumnIndex(), cell.getColumnIndex()), velocityTTestSheet);

						cell = row.createCell(4);
						cell.setCellStyle(style11);
						cell.setCellValue(sortedGroupFiles.get(i).getVelocityA());
						setRegionBorderThin(new CellRangeAddress(rowNumber, rowNumber, cell.getColumnIndex(), cell.getColumnIndex()), velocityTTestSheet);
						...
						cell.setCellStyle(style11);
						cell.setCellValue(sortedGroupFiles.get(i).getVelocityB());
						setRegionBorderThin(new CellRangeAddress(rowNumber, rowNumber, cell.getColumnIndex(), cell.getColumnIndex()), velocityTTestSheet);

						// Put borders around all of the significant, slight and equal cells
						setRegionBorderThin(new CellRangeAddress(rowNumber, rowNumber, 5, 5), velocityTTestSheet);
						setRegionBorderThin(new CellRangeAddress(rowNumber, rowNumber, 6, 6), velocityTTestSheet);
						setRegionBorderThin(new CellRangeAddress(rowNumber, rowNumber, 7, 7), velocityTTestSheet);
						if (BIASTtestConfigPageController.getShowSlightAndEqualTstatColumns())
						{
							setRegionBorderThin(new CellRangeAddress(rowNumber, rowNumber, 8, 8), velocityTTestSheet);
							setRegionBorderThin(new CellRangeAddress(rowNumber, rowNumber, 9, 9), velocityTTestSheet);
						}
						
						if (BIASTtestConfigPageController.getShowSlightAndEqualTstatColumns())
						{
							// Color appropriate significance column (5 through 9)
							if (sortedGroupFiles.get(i).getAbsMeanVelocityDiff() == 0)
							{
								// Is equal
								cell = row.createCell(7);
								cell.setCellStyle(style14);
								cell.setCellValue("");
								setRegionBorderThick(new CellRangeAddress(rowNumber, rowNumber, cell.getColumnIndex(), cell.getColumnIndex()), velocityTTestSheet);
							}
							else if (sortedGroupFiles.get(i).getVelocitySignificant())
							{
								if (sortedGroupFiles.get(i).getVelocityA() > sortedGroupFiles.get(i).getVelocityB())
								{
						...
						else
						{
							...
							if (sortedGroupFiles.get(i).getVelocitySignificant())
							{
								if (sortedGroupFiles.get(i).getVelocityA() > sortedGroupFiles.get(i).getVelocityB())
								{
									// Is significant and A > B
				...
				// Set thick borders
				setRegionBorderThick(new CellRangeAddress(lastClusterRowStart, rowNumber, 4, 4), velocityTTestSheet);
				if (BIASTtestConfigPageController.getShowSlightAndEqualTstatColumns())
				{
					setRegionBorderThick(new CellRangeAddress(lastClusterRowStart, rowNumber, 10, 10), velocityTTestSheet);
				}
				else
				{
					setRegionBorderThick(new CellRangeAddress(lastClusterRowStart, rowNumber, 8, 8), velocityTTestSheet);
				}
			...
			if (byTypeTTestResults.size() > 0)
			{
				...
				cell = row.createCell(0);
				cell.setCellStyle(style9);
				cell.setCellValue("Velocity - Type Tests");
				...
				{
					if ((sortedTypeFiles.get(i).getVelocityA() != null)  && (sortedTypeFiles.get(i).getVelocityB() != null))
					{
						resultsSetSize++;
						rowNumber++;
						row = velocityTTestSheet.createRow(rowNumber);
						...
						cell = row.createCell(2);
						cell.setCellStyle(style5);
						if (sortedTypeFiles.get(i).getVelocityB() > sortedTypeFiles.get(i).getVelocityA())
						{
							cell.setCellValue(sortedTypeFiles.get(i).getAbsMeanVelocityDiff());
						}
						else
						{
							cell.setCellValue(-1 * sortedTypeFiles.get(i).getAbsMeanVelocityDiff());
						}
						setRegionBorderThin(new CellRangeAddress(rowNumber, rowNumber, cell.getColumnIndex(), cell.getColumnIndex()), velocityTTestSheet);

						cell = row.createCell(3);
						cell.setCellStyle(style5);
						if (Double.isInfinite(sortedTypeFiles.get(i).getVelocityTValue()))
						{
							cell.setCellValue("infinity");
						}
						else if (Double.isNaN(sortedTypeFiles.get(i).getVelocityTValue()))
						{
							cell.setCellValue(0.00);
						}
						else if (sortedTypeFiles.get(i).getVelocityB() > sortedTypeFiles.get(i).getVelocityA())
						{
							cell.setCellValue(sortedTypeFiles.get(i).getVelocityTValue());
						}
						else
						{
							cell.setCellValue(-1 * sortedTypeFiles.get(i).getVelocityTValue());
						}
						setRegionBorderThin(new CellRangeAddress(rowNumber, rowNumber, cell.getColumnIndex(), cell.getColumnIndex()), velocityTTestSheet);

						cell = row.createCell(4);
						cell.setCellStyle(style11);
						cell.setCellValue(sortedTypeFiles.get(i).getVelocityA());
						setRegionBorderThin(new CellRangeAddress(rowNumber, rowNumber, cell.getColumnIndex(), cell.getColumnIndex()), velocityTTestSheet);
						...
						cell.setCellStyle(style11);
						cell.setCellValue(sortedTypeFiles.get(i).getVelocityB());
						setRegionBorderThin(new CellRangeAddress(rowNumber, rowNumber, cell.getColumnIndex(), cell.getColumnIndex()), velocityTTestSheet);
						...
						{
							// Color appropriate significance column (5 through 9)
							if (sortedTypeFiles.get(i).getAbsMeanVelocityDiff() == 0)
							{
								// Is equal
								cell = row.createCell(7);
								cell.setCellStyle(style14);
							...
							else if (sortedTypeFiles.get(i).getVelocitySignificant())
							{
								if (sortedTypeFiles.get(i).getVelocityA() > sortedTypeFiles.get(i).getVelocityB())
								{
									// Is significant and A > B
									cell = row.createCell(5);
									cell.setCellStyle(style12);
						...
						{
							// Color appropriate significance column (5 through 7)
							if (sortedTypeFiles.get(i).getVelocitySignificant())
							{
								if (sortedTypeFiles.get(i).getVelocityA() > sortedTypeFiles.get(i).getVelocityB())
								{
									// Is significant and A > B
		...
		for(int i = 0; i < 11; i++) 
		{
			if (i <= 4)
			{
				velocityTTestSheet.autoSizeColumn(i);
				int currentWidth = velocityTTestSheet.getColumnWidth(i);
				currentWidth += 360;
				velocityTTestSheet.setColumnWidth(i, currentWidth);
			}
			else if ((i == 10) && (BIASTtestConfigPageController.getShowSlightAndEqualTstatColumns()))
			{
				velocityTTestSheet.autoSizeColumn(i);
				int currentWidth = Math.max(velocityTTestSheet.getColumnWidth(4), velocityTTestSheet.getColumnWidth(10));
				currentWidth += 360;
				velocityTTestSheet.setColumnWidth(4, currentWidth);
				velocityTTestSheet.setColumnWidth(10, currentWidth);
			}
			else if ((i == 8) && (!BIASTtestConfigPageController.getShowSlightAndEqualTstatColumns()))
			{
				velocityTTestSheet.autoSizeColumn(i);
				int currentWidth = Math.max(velocityTTestSheet.getColumnWidth(4), velocityTTestSheet.getColumnWidth(8));
				currentWidth += 360;
				velocityTTestSheet.setColumnWidth(4, currentWidth);
				velocityTTestSheet.setColumnWidth(8, currentWidth);
			}
			else
			{
				velocityTTestSheet.setColumnWidth(i, 4100);
			}
		}
		resultsMessage += "\nWrote "+resultsSetSize+ " velocity t-test results";
	}
	...
	public static XSSFWorkbook getWorkbook1()
	{
		return workbook;
	}
	...
	public static int getResultsSetSize1()
	{
